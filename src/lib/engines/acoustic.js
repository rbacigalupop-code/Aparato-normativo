...
// ─── Estimación de Rw por composición de elementos ────────────────────────
export function estimarRwComposicion(elementos) {
  if (!elementos || !elementos.length) return null

  // Fórmula de Sabine simplificada para múltiples capas
  // Rw_total = 20*log10(suma(1/10^(Rwi/10)))
  let sumaInversas = 0
  for (const elem of elementos) {
    const rw = parseFloat(elem.rw) || 0
    if (rw > 0) {
      sumaInversas += Math.pow(10, -rw / 10)
    }
  }

  if (sumaInversas <= 0) return null
  const rwTotal = -20 * Math.log10(sumaInversas)

  return {
    rwEstimado: Math.round(rwTotal * 10) / 10,
    elementos: elementos.length,
    metodo: 'Sabine simplificado',
  }
}
...
// ─── Cálculo de mejora acústica por elemento adicional ──────────────────────
export function calcularMejoraAcustica(rwBase, elementosAdicionales) {
  ...
  let sumaInversas = Math.pow(10, -rw0 / 10)
  ...
  if (sumaInversas <= 0) return null
  const rwMejorado = -20 * Math.log10(sumaInversas)

  return {
    rwBase: Math.round(rw0 * 10) / 10,
    rwMejorado: Math.round(rwMejorado * 10) / 10,
    mejora: Math.round((rwMejorado - rw0) * 10) / 10,
