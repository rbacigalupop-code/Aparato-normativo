...
-- Version 2: Sin dependencias de tabla 'proyectos' existente
...
CREATE INDEX IF NOT EXISTS idx_org_propietario ON organizaciones(propietario_id);
CREATE INDEX IF NOT EXISTS idx_org_slug ON organizaciones(slug);
...
CREATE INDEX IF NOT EXISTS idx_perfil_user ON perfiles_usuario(user_id);
CREATE INDEX IF NOT EXISTS idx_perfil_org ON perfiles_usuario(organizacion_id);
CREATE INDEX IF NOT EXISTS idx_perfil_rol ON perfiles_usuario(rol);

-- TABLA 3: tokens_legado (backward compatibility con sistema anterior)
...
-- TABLA 4: registro_auditoria (audit trail de cambios)
...
-- TABLA 5: proyectos (si no existe, la creamos)
...
CREATE INDEX IF NOT EXISTS idx_proyecto_token ON proyectos(token);
...
CREATE POLICY "Usuarios ven sus propios proyectos"
  ...
  USING (auth.uid() = user_id OR token IS NOT NULL);

CREATE POLICY "Usuarios pueden crear proyectos"
  ON proyectos FOR INSERT
  WITH CHECK (auth.uid() = user_id OR token IS NOT NULL);
